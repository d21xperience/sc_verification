<script setup>
import AutoComplete from 'primevue/autocomplete';

import { ref, watch } from 'vue';
import axios from "axios";
import { debounce, size } from "lodash";
const filteredSekolah = ref([])
const sekolah = ref(true)
const baseUrl = '/api/getHasilPencarian'
const selectedSekolah = ref()
const loadingSearchSekolah = ref(false)
const fetchData = async (e) => {
    // console.log(e.query)
    if (e.query.length <= 3) return
    loadingSearchSekolah.value = true
    try {
        const res = await axios.get(baseUrl, {
            params: {
                keyword: e.query
            }
        })
        // console.log(res.data)
        // console.log(res.data.map((item) => item.nama_sekolah))
        filteredSekolah.value = res.data.map((item) => `${item.npsn} - ${item.nama_sekolah}`)
        loadingSearchSekolah.value = false
    } catch (error) {
        console.error(error)
    }

}



const cariSekolah = debounce(fetchData, 500)
watch(selectedSekolah, (newFilteredSekolah, oldFilteredsekolah) => {
    // console.log(newFilteredSekolah.length)
    if (newFilteredSekolah == null) {
        sekolah.value = true
    } else {
        sekolah.value = false
    }
})


// membuat login form
const username = ref("");
const password = ref("");

const register = async () => {
    try {
        const response = await axios.post('http://localhost:8080/register', {
            username: username.value,
            password: password.value,
        })
        // localStorage.setItem('token', response.data.token)
        this.$router.push('/')
    } catch (error) {
        console.error('Login failed', error)
    }
}

</script>



<template>
    <div class=" bg-slate-900 flex items-center md:h-screen p-4">
        <div class="w-full max-w-xl max-md:max-w-xl mx-auto">
            <div class="bg-white grid gap-16 w-full sm:p-8 p-6 shadow-md rounded-md overflow-hidden">
                <form class="w-full justify-center">
                    <div class="mb-8">
                        <h3 class="text-gray-800 text-2xl font-bold">Register</h3>
                        <p>Register diperuntukan untuk admin Sekolah</p>
                    </div>

                    <div class="space-y-6">
                        <div>
                            <label class="text-gray-800 text-sm mb-2 block">NPSN / Nama Sekolah</label>
                            <div class="relative flex items-center">
                                <input name="email" type="email" required
                                    class="bg-white border border-gray-300 w-full text-sm text-gray-800 pl-4 pr-10 py-2.5 rounded-md outline-blue-500"
                                    placeholder="Disi nama NPSN atau Nama Sekolah" />
                                <svg xmlns="http://www.w3.org/2000/svg" fill="#bbb" stroke="#bbb"
                                    class="w-4 h-4 absolute right-4" viewBox="0 0 682.667 682.667">
                                    <defs>
                                        <clipPath id="a" clipPathUnits="userSpaceOnUse">
                                            <path d="M0 512h512V0H0Z" data-original="#000000"></path>
                                        </clipPath>
                                    </defs>
                                    <g clip-path="url(#a)" transform="matrix(1.33 0 0 -1.33 0 682.667)">
                                        <path fill="none" stroke-miterlimit="10" stroke-width="40"
                                            d="M452 444H60c-22.091 0-40-17.909-40-40v-39.446l212.127-157.782c14.17-10.54 33.576-10.54 47.746 0L492 364.554V404c0 22.091-17.909 40-40 40Z"
                                            data-original="#000000"></path>
                                        <path
                                            d="M472 274.9V107.999c0-11.027-8.972-20-20-20H60c-11.028 0-20 8.973-20 20V274.9L0 304.652V107.999c0-33.084 26.916-60 60-60h392c33.084 0 60 26.916 60 60v196.653Z"
                                            data-original="#000000"></path>
                                    </g>
                                </svg>
                            </div>
                        </div>
                        <div>
                            <label class="text-gray-800 text-sm mb-2 block">Email Dapodik</label>
                            <div class="relative flex items-center">
                                <input name="email" type="email" required
                                    class="bg-white border border-gray-300 w-full text-sm text-gray-800 pl-4 pr-10 py-2.5 rounded-md outline-blue-500"
                                    placeholder="Isi email yang terdaftar di Dapodik" />
                                <svg xmlns="http://www.w3.org/2000/svg" fill="#bbb" stroke="#bbb"
                                    class="w-4 h-4 absolute right-4" viewBox="0 0 682.667 682.667">
                                    <defs>
                                        <clipPath id="a" clipPathUnits="userSpaceOnUse">
                                            <path d="M0 512h512V0H0Z" data-original="#000000"></path>
                                        </clipPath>
                                    </defs>
                                    <g clip-path="url(#a)" transform="matrix(1.33 0 0 -1.33 0 682.667)">
                                        <path fill="none" stroke-miterlimit="10" stroke-width="40"
                                            d="M452 444H60c-22.091 0-40-17.909-40-40v-39.446l212.127-157.782c14.17-10.54 33.576-10.54 47.746 0L492 364.554V404c0 22.091-17.909 40-40 40Z"
                                            data-original="#000000"></path>
                                        <path
                                            d="M472 274.9V107.999c0-11.027-8.972-20-20-20H60c-11.028 0-20 8.973-20 20V274.9L0 304.652V107.999c0-33.084 26.916-60 60-60h392c33.084 0 60 26.916 60 60v196.653Z"
                                            data-original="#000000"></path>
                                    </g>
                                </svg>
                            </div>
                        </div>
                        <div>
                            <label class="text-gray-800 text-sm mb-2 block">Password</label>
                            <div class="relative flex items-center">
                                <input name="password" type="password" required
                                    class="bg-white border border-gray-300 w-full text-sm text-gray-800 pl-4 pr-10 py-2.5 rounded-md outline-blue-500"
                                    placeholder="Masukan password" />
                                <svg xmlns="http://www.w3.org/2000/svg" fill="#bbb" stroke="#bbb"
                                    class="w-4 h-4 absolute right-4 cursor-pointer" viewBox="0 0 128 128">
                                    <path
                                        d="M64 104C22.127 104 1.367 67.496.504 65.943a4 4 0 0 1 0-3.887C1.367 60.504 22.127 24 64 24s62.633 36.504 63.496 38.057a4 4 0 0 1 0 3.887C126.633 67.496 105.873 104 64 104zM8.707 63.994C13.465 71.205 32.146 96 64 96c31.955 0 50.553-24.775 55.293-31.994C114.535 56.795 95.854 32 64 32 32.045 32 13.447 56.775 8.707 63.994zM64 88c-13.234 0-24-10.766-24-24s10.766-24 24-24 24 10.766 24 24-10.766 24-24 24zm0-40c-8.822 0-16 7.178-16 16s7.178 16 16 16 16-7.178 16-16-7.178-16-16-16z"
                                        data-original="#000000"></path>
                                </svg>
                            </div>
                        </div>
                        <div class="flex items-center">
                            <input id="remember-me" name="remember-me" type="checkbox"
                                class="h-4 w-4 shrink-0 text-blue-600 focus:ring-blue-500 border-gray-300 rounded-md" />
                            <label for="remember-me" class="text-gray-800 ml-3 block text-sm">
                                Saya menyetujui <a href="javascript:void(0);"
                                    class="text-blue-600 font-semibold hover:underline ml-1">Persyaratan dan Aturan</a>
                            </label>
                        </div>
                    </div>

                    <div class="!mt-8">
                        <button type="button"
                            class="w-full py-2.5 px-4 text-sm tracking-wider font-semibold rounded-md bg-blue-600 hover:bg-blue-700 text-white focus:outline-none">
                            Buat Akun
                        </button>
                    </div>
                    <p class="text-gray-800 text-sm mt-6 text-center">Sudah punya akun ? <RouterLink
                            :to="{ name: 'login' }" class="text-blue-600 font-semibold hover:underline ml-1">Login
                            disini</RouterLink>
                    </p>
                </form>
            </div>
        </div>
    </div>
</template>